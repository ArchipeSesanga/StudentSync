...

    {
        ...
        public IActionResult DeleteConfirmed(string id)
        {
            try
            {
                ...
                if (consumer == null)
                {
                    TempData["ErrorMessage"] = "Consumer not found.";
                    return RedirectToAction(nameof(Index));
                }

                _consumerRepo.Delete(consumer);
                TempData["SuccessMessage"] = "Consumer deleted successfully.";
                return RedirectToAction(nameof(Index));
            }
            catch (DbUpdateException)
            {
                TempData["ErrorMessage"] = "This consumer cannot be deleted because it is referenced elsewhere.";
                return RedirectToAction("Delete", new { id = id });
            }
            catch (Exception)
            {
                TempData["ErrorMessage"] = "An unexpected error occurred while deleting the consumer.";
                return RedirectToAction("Delete", new { id = id });
            }
        }
